...
def train_model_with_ranges(user_id: str, dataset_id: str, ranges: dict) -> dict:
    ...
    # Initialize list to collect new feature Series (Performance optimization)
    new_feature_series = []
    ...
    # Add station count features - collect Series instead of adding columns directly
    for station in list(stations)[:50]:  # Limit to prevent memory issues
        station_cols = [col for col in available_features if f"_S{station[1:]}_" in col]
        if station_cols:
            # Calculate features as Series and collect them
            count_series = df[station_cols].notna().sum(axis=1)
            count_series.name = f"{station}_count"
            new_feature_series.append(count_series)
            
            mean_series = df[station_cols].mean(axis=1)
            mean_series.name = f"{station}_mean"
            new_feature_series.append(mean_series)
    ...
    # Process line features - collect Series instead of adding columns directly
    for line in list(lines):
        line_cols = [col for col in available_features if col.startswith(f"{line}_")]
        if line_cols:
            # Calculate all line-based features as Series and collect them
            numeric_count_series = df[line_cols].notna().sum(axis=1)
            numeric_count_series.name = f"{line}_numeric_count"
            new_feature_series.append(numeric_count_series)
            
            positive_count_series = (df[line_cols] > 0).sum(axis=1)
            positive_count_series.name = f"{line}_positive_count"
            new_feature_series.append(positive_count_series)
            
            negative_count_series = (df[line_cols] < 0).sum(axis=1)
            negative_count_series.name = f"{line}_negative_count"
            new_feature_series.append(negative_count_series)
            
            zero_count_series = (df[line_cols] == 0).sum(axis=1)
            zero_count_series.name = f"{line}_zero_count"
            new_feature_series.append(zero_count_series)
            
            missing_count_series = df[line_cols].isna().sum(axis=1)
            missing_count_series.name = f"{line}_missing_count"
            new_feature_series.append(missing_count_series)
    
    # Concatenate all new features at once (Performance optimization)
    if new_feature_series:
        new_features_df = pd.concat(new_feature_series, axis=1)
        df = pd.concat([df, new_features_df], axis=1)
    
    # Update feature list to include engineered features
    engineered_features = [series.name for series in new_feature_series]
    all_features = available_features + engineered_features
    ...
    if not simulation_df.empty:
        ...
        # Store to data_store directory instead of Redis
        ...
        simulation_df_processed.to_parquet(simulation_path, index=False)
        
        log.info(f"Simulation data saved to: {simulation_path}")
